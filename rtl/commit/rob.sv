module rob #(
    ...
    assign free_phys =
    (rob_mem[head].rd_arch != 0) ? rob_mem[head].old_phys : '0;
...
always_ff @(posedge clk) begin
    if (wb_en)
        $display("WB -> ROB idx=%0d", wb_idx);

    if (commit_valid)
        $display("COMMIT -> idx=%0d", commit_idx);
end

// ================= ROB ASSERTIONS =================

// Commit only if entry is valid
assert property (@(posedge clk)
    commit_valid |-> rob_mem[head].valid);

// Commit only if instruction is done
assert property (@(posedge clk)
    commit_valid |-> rob_mem[head].done);

// No allocation when ROB is full
assert property (@(posedge clk)
    alloc_en |-> !rob_full);
