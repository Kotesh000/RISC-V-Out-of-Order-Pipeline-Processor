...
assign instr_valid =
    ...
    logic [5:0]  ex_dst;
    logic [3:0] id_alu_op;
    logic issue_is_load, issue_is_store;
    ...
    logic [$clog2(8)-1:0] ren_lsq_idx;
    logic [3:0] issue_alu_op;

    assign flush_idx = rob_idx_ex;
...
reservation_station u_rs (
    ...
    .dispatch_alu_op(id_alu_op),
    .issue_alu_op(issue_alu_op),
    .issue_is_load(issue_is_load),
    .issue_is_store(issue_is_store),
    .dispatch_is_load(id_is_load),
    .dispatch_is_store(id_is_store),
...
always_ff @(posedge clk or posedge rst) begin
    if (rst || flush) begin
        ...
        ex_alu_op <= issue_alu_op; 
        ex_imm       <= issue_imm;
        ex_is_load  <= issue_is_load;
        ex_is_store <= issue_is_store;
    end
    else begin
        ex_valid <= 1'b0;
    end
...
always_ff @(posedge clk or posedge rst) begin
    if (rst || flush)
        rob_idx_ex <= '0;
    else if (alu_grant && rs_issue_valid)
...
memory_system u_mem_system (
    ...
    .mem_alloc_en(rs_dispatch_en && (id_is_load || id_is_store)),
    .mem_is_store(ex_is_store),
    .mem_rd_phys(ren_rd_phys),
    .mem_rob_idx(alloc_rob_idx),
    .mem_lsq_full(mem_lsq_full),
    .flush(flush),
    .mem_exec_idx(ren_lsq_idx),

    .mem_exec_en (ex_valid && (ex_is_load || ex_is_store)),
    .rs1_val(ex_a),
    .imm(ex_imm),
    .rs2_val(ex_b),
    ...
    .instr_valid (ren_valid),
    .rob_full    (rob_full),
    .rs_full     (rs_full),
    .fl_empty    (!alloc_valid),
    .lsq_full    (mem_lsq_full),
    .is_mem_instr(id_is_load || id_is_store),
...
always_ff @(posedge clk or posedge rst) begin
    if (rst || flush)
        ren_lsq_idx <= '0;
    else if (rs_dispatch_en && (id_is_load || id_is_store))
        ren_lsq_idx <= lsu_idx;
end
...
// EX stage must come from issue
assert property (@(posedge clk)
    ex_valid |-> $past(alu_grant && rs_issue_valid));
